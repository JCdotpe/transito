...
//        $prestasi->setCellValue('F' . $numerNombre, 'APELLIDOS');
        $prestasi->setCellValue('F' . $numerNombre, 'NOMBRES');
        $prestasi->setCellValue('G' . $numerNombre, 'AULA');
        $prestasi->setCellValue('H' . $numerNombre, 'ASISTENCIA');

        
//        ($this->input->get('local') != "") ? $local = $this->input->get('local') : $local = 0;
        $data = $this->modeloMonitor->getDetalleRegistro($_REQUEST['local']);
...
//            $prestasi->setCellValue('F' . $rowexcel, $row['APELLIDOS'])->getStyle('F' . $rowexcel)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
            $prestasi->setCellValue('F' . $rowexcel, $row['NOMBRES'])->getStyle('F' . $rowexcel)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
            $prestasi->setCellValue('G' . $rowexcel, $row['AULA']);
            $prestasi->setCellValue('H' . $rowexcel, $row['ASISTENCIA']);
        }
